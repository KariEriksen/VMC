...
class Hamiltonian:
    ...
    def laplacian_analytic_non_interacting(self, positions):
    ...
    def local_energy_weak_interaction_numerical(self, positions):
        """Return the local energy."""
        # Run with numerical expression for kinetic energy
        k = self.laplacian_numerical(positions)/self.s.wavefunction(positions)
        p = self.trap_potential_energy(positions)
        energy = -0.5*k + p

        return energy

    def local_energy_weak_interaction_analytical(self, positions):
        """Return the local energy."""
    ...
    def local_energy_times_wf_non_interaction(self, positions):
        """Return local energy times the derivative of wave equation."""
        energy = self.local_energy_weak_interaction_numerical(positions)
        energy_times_wf = self.s.gradient_wavefunction(positions)*energy

        return energy_times_wf

    def local_energy_times_wf_weak_interaction(self, positions):
        """Return local energy times the derivative of wave equation."""
        energy = self.local_energy_weak_interaction_numerical(positions)
        energy_times_wf = self.s.gradient_wavefunction(positions)*energy
